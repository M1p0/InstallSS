...
# ---- 对比测试用：额外开一个 SS-2022 入站，与 Reality 并存 -------------
# 设为 "yes" 会在 SS_PORT 上多开一个 Shadowsocks-2022 节点，方便你两个
# 节点来回切、实测同一线路同一机器下哪个延迟低。测完想关掉设回 "no" 即可。
ENABLE_SS="yes"
SS_PORT="8443"
...
# x25519 输出格式随版本变化（这是个大坑，分三代）：
#   老版:        "Private key: xxx" / "Public key: yyy"
#   中间态:      "PrivateKey: xxx"  / "PublicKey: yyy"
#   新版(25.3.6+): "PrivateKey: xxx" / "Password: yyy" / "Hash32: zzz"
#       ↑ 官方把"公钥"改名成 Password(它就是客户端要填的那个值)，
#         REALITY 用不到 Hash32。所以公钥这行要同时匹配 Public key/PublicKey/Password。
X25519_OUT="$("$XRAY_BIN" x25519)"
PRIVATE_KEY="$(echo "$X25519_OUT" | grep -Ei 'PrivateKey|Private key' | awk -F': ' '{print $2}' | tr -d ' \r')"
# 公钥：新版叫 Password，老版叫 Public key/PublicKey；用 Hash32 之外的那个。
# 注意排除 Hash32（grep 时不含它），避免误取。
PUBLIC_KEY="$(echo "$X25519_OUT" | grep -Ei 'Password|PublicKey|Public key' | grep -vi 'Hash32' | awk -F': ' '{print $2}' | tr -d ' \r')"

# 兜底校验：万一格式又变导致没抓到，立即报错而不是写出空 key 的坏配置
if [[ -z "$PRIVATE_KEY" || -z "$PUBLIC_KEY" ]]; then
  echo "[!] 未能从 'xray x25519' 输出解析出密钥，原始输出如下：" >&2
  echo "----------------------------------------" >&2
  echo "$X25519_OUT" >&2
  echo "----------------------------------------" >&2
  echo "[!] 密钥格式可能又变了。请把上面输出贴出来。已停止，避免写出坏配置。" >&2
  exit 1
fi
...
# SS-2022 密码：必须是 Base64 编码的随机字节（aes-256 用 32 字节）。
...
# 按需构造 SS-2022 入站片段（带前导逗号，接在 Reality inbound 之后）
SS_INBOUND_JSON=""
if [[ "$ENABLE_SS" == "yes" ]]; then
  SS_INBOUND_JSON=$(cat <<SSEOF
,
    {
      "listen": "::",
      "port": ${SS_PORT},
...
# 收集所有要放行的端口（Reality 端口 + 可选 SS 端口）
PORTS_TO_OPEN=( "${LISTEN_PORT}" )
[[ "$ENABLE_SS" == "yes" ]] && PORTS_TO_OPEN+=( "${SS_PORT}" )

# ---- 5. SELinux 端口处理（仅 RHEL 系且端口非 443 时需要）-------------
# SELinux 默认策略已包含 tcp 443 (http_port_t)，用 443 无需处理。
# 非 443 端口需加入 http_port_t，否则 Xray 无法 bind（Permission denied）。
if [[ "$OS_FAMILY" == "rhel" ]] && command -v getenforce >/dev/null 2>&1; then
  if [[ "$(getenforce)" == "Enforcing" ]]; then
    ...
    for p in "${PORTS_TO_OPEN[@]}"; do
      if [[ "$p" != "443" ]]; then
        echo "[*] SELinux: 放行 ${p}/tcp 到 http_port_t ..."
        semanage port -a -t http_port_t -p tcp "${p}" 2>/dev/null \
          || semanage port -m -t http_port_t -p tcp "${p}" 2>/dev/null \
          || echo "    [!] semanage 设置 ${p} 失败，若起不来请手动处理"
      else
        echo "[*] SELinux: 443 已在 http_port_t 内，无需处理"
      fi
    done
  fi
fi
...
FW_DONE="no"
USE_UFW="no"; USE_FWD="no"
if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -q "Status: active"; then USE_UFW="yes"; fi
if command -v firewall-cmd >/dev/null 2>&1 && systemctl is-active --quiet firewalld; then USE_FWD="yes"; fi

open_port() {  # $1=端口 $2=协议(tcp/udp)
  local port="$1" proto="$2"
  if [[ "$USE_UFW" == "yes" ]]; then
    ufw allow "${port}/${proto}" >/dev/null 2>&1 || true; FW_DONE="yes"
  fi
  if [[ "$USE_FWD" == "yes" ]]; then
    firewall-cmd --permanent --add-port="${port}/${proto}" >/dev/null 2>&1 || true; FW_DONE="yes"
  fi
}

# Reality：tcp
open_port "${LISTEN_PORT}" tcp
# SS：tcp + udp（入站开了 udp）
if [[ "$ENABLE_SS" == "yes" ]]; then
  open_port "${SS_PORT}" tcp
  open_port "${SS_PORT}" udp
fi
[[ "$USE_FWD" == "yes" ]] && firewall-cmd --reload >/dev/null 2>&1 || true

if [[ "$FW_DONE" == "yes" ]]; then
  echo "[*] 已放行端口：${PORTS_TO_OPEN[*]}（SS 端口含 udp）"
else
  echo "[*] 未检测到活动的 ufw/firewalld。若云厂商有安全组，请去控制台放行："
  echo "    ${LISTEN_PORT}/tcp$( [[ "$ENABLE_SS" == "yes" ]] && echo "，以及 ${SS_PORT}/tcp + ${SS_PORT}/udp" )"
fi
...
# SS-2022 分享链接（SIP002: ss://base64(method:password)@host:port#tag）
if [[ "$ENABLE_SS" == "yes" ]]; then
  SS_USERINFO_B64="$(printf '%s' "${SS_METHOD}:${SS_PASSWORD}" | base64 -w0 2>/dev/null || printf '%s' "${SS_METHOD}:${SS_PASSWORD}" | base64)"
  SS_LINK="ss://${SS_USERINFO_B64}@${SERVER_IP}:${SS_PORT}#SS2022-${SERVER_IP}"
fi
...
==========================================================================
$( [[ "$ENABLE_SS" == "yes" ]] && cat <<SSINFO

==========================================================================
  【对比测试节点】Shadowsocks-2022（与 Reality 并存，同机同线路）
==========================================================================
  地址 (address) : ${SERVER_IP}
  端口 (port)    : ${SS_PORT}
  加密 (method)  : ${SS_METHOD}
  密码 (password): ${SS_PASSWORD}
  传输           : tcp + udp

  —— 一键导入链接 ——
  ${SS_LINK}

  测试方法：把这个 SS 节点和上面的 Reality 节点都加进客户端，
  来回切换，访问同样的网站/测同样的延迟。同机同线路同时段，
  数字摆出来就知道是不是协议差异。测完想撤掉 SS：
  把脚本顶部 ENABLE_SS 设为 "no" 重跑，或手动删该 inbound 重启 xray。
==========================================================================
SSINFO
)
EOF
